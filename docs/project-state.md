...
| S002 | Every project detail lists intended capabilities with verified, partial, blocked, or missing state | verified | S001 | G001 |
...
refuses missing or malformed inventories, generates the committed TypeScript snapshot, and reports the
project/feature denominator in `--check` mode so a stale or empty result cannot look current.
